...
class FlatbuildTrainer:
    ...
    def train(self) -> TrainArtifacts:
        ...
        try:
            # Use saved epoch_index + 1, fallback to step-based calculation
            if self.epoch_index >= 0 and self.global_step > 0:
                start_epoch = self.epoch_index + 1
                logger.info(f"Resuming from step {self.global_step}, epoch {self.epoch_index}")
            else:
                start_epoch = 0
